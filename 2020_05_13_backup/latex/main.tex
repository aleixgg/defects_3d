...
\begin{align}
 [D, Q^+_\a ] & = -\tfrac{i}{2} Q^+_\a, \\
 [D, Q^-_\ad] & = -\tfrac{i}{2} Q^-_\ad, \\
 [R, Q^+_\a ] & =  Q^+_\a, \\
 [R, Q^-_\ad] & = -Q^-_\ad, \\
...
\begin{align}
 ...
 \{ S^{\ad+}, Q^-_\bd \} 
   & = \delta^\a_{\ph \a\b} \left(iD + \frac{d-1}{2} R \right) 
     + (m_{\mu\nu})_\b^{\ph \b\a} M_{\mu\nu}
     + \ldots, \\
 \{ S^{\a-}, Q^+_\b \} 
   & = \delta^\ad_{\ph \ad\bd} \left(iD - \frac{d-1}{2} R \right) 
...
\begin{align}
 & [D, \phi_1(0)] = - i \Delta_\phi, \\
 & [D, \phi_2(0)] = - i \Delta_\phi, \\
...
\begin{align}
 & [S^{\a-}, \phi_1(x_1)]
 = i x_1^\mu \bar \Sigma_\mu^{\ad\a} [Q^-_\ad, \phi_1(x_1)], \\
 & [S^{\ad+}, \phi_2(x_2)]
 = i x_2^\mu \bar \Sigma_\mu^{\ad\a} [Q^+_\a,  \phi_2(x_2)].
\end{align}
...
\begin{split}
 ...
 & = 
 \left(
    i x_1^\mu \bar \Sigma_\mu^{\ad\a} [Q^-_\ad, \phi_1(x_1)] [Q^+_\ad, \phi_2(x_2)]
  - i x_2^\mu \bar \Sigma_\mu^{\ad\a} [Q^-_\ad, \phi_1(x_1)] [Q^+_\a,  \phi_2(x_2)]
  + 4 \Delta_\phi \, \phi_1(x_1) \phi_2(x_2)
 \right) |0\rangle
\end{split}
\end{align}
...
\begin{align}
 ...
 & = -i \Sigma^\mu_{1\ad} \partial_\mu^{x_1} \langle \phi_1(x_1) \phi_2(x_2) \rangle. 
\end{align}
Explicitly
\begin{align}
...
\begin{align}
 ...
 = \left( \Delta_\phi \phi_1(x_1) - i x_1^2 \{ Q_2^-, [Q_1^-, \phi_1(x_1)] \right)|0\rangle.
\end{align}
Using Ward identities we get
\begin{align}
 \frac{C_{\text{bulk,ferm}} \langle \phi_1(x_1) \phi_2(x_2) \rangle}
      {\langle \phi_1(x_1) \phi_2(x_2) \rangle}
 = -\xi  f'(\xi).
...
\end{split}
\end{align}
The rest are a bit more intricate, but all in all we get:
\begin{align}
 \frac{C_{\text{bulk,ferm}} \langle \phi_1(x_1) \phi_2(x_2) \rangle}
      {\langle \phi_1(x_1) \phi_2(x_2) \rangle}
 = 4 \xi  f'(\xi )
...
We should still do the boundary in $2d$, but it seems the formulas will be:
...
\begin{align}
 ...
 M_{ab}.
\end{align}
where $i,j = 1, \ldots, p$ and $a,b= p+1, \ldots, d$, $p$ is the dimension of the defect and $q  = d-p$ is the codimension.
The defect Casimir is obtained from the full one restricting to the defect operators.
...
\begin{align}
 C_{\text{def}} = 
  ...
  + \frac12 [ S^{1-}, Q^+_1].
\end{align}
Acting with it on an operator with quantum numbers $\Delta, \ell$, where $\ell$ is the eigenvalue of $M_{12} + R$ gives the eigenvalue:
\begin{align}
 \lambda_C 
 = \Delta^2
 - \ell^2.
...
\end{split}
\end{align}
Acting at point 1 only the contribution from $R$-symmetry survives
\begin{align}
\begin{split}
 [C_{\text{ferm}}, \phi_1(x_1) ] |0\rangle
 & = \left( - \Delta_\phi^2 + \Delta_\phi \right) \phi_1(x_1) |0\rangle.
\end{split}
\end{align}
