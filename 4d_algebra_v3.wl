...
terms = (
	+ term1[\[ScriptCapitalD]]
	- I term1[\[ScriptCapitalM][3,4]]
	+ I \[CapitalDelta]p term1[\[ScriptCapitalM][1,2]]
	- 1/2\[CapitalDelta]p^2 id
)


terms = (
	+ term1[\[ScriptCapitalM][1, 2]]
	(*- 1/2\[CapitalDelta]p^2 id*)
)
...
\[Chi]v = (x12Sq + 2x12Orth) / (x1Orth x2Orth);
\[Eta]v = x12Orth / (x1Orth x2Orth);
der[f_[\[Chi], \[Eta]], a_] := (
	+ D[f[\[Chi], \[Eta]], \[Chi]] der[\[Chi]v, a]
	+ D[f[\[Chi], \[Eta]], \[Eta]] der[\[Eta]v, a]
);
der[x12Sq, a_] := der[x12SqV, a];
der[x1Orth, a_] := der[x1OrthV, a];
der[x2Orth, a_] := der[x2OrthV, a];
der[x12Orth, a_] := der[x12OrthV, a];
der[x[i_][\[Mu]_], x[j_][\[Nu]_]] := \[Delta][i, j] \[Delta][\[Mu], \[Nu]];
pref = x1Orth^(-\[CapitalDelta]p) x2Orth^(-\[CapitalDelta]p);
expr = terms /. {
	\[ScriptD][i_][\[Mu]_] :> der[pref f[\[Chi]v, \[Eta]v], x[i][\[Mu]]],
	id :> pref f[\[Chi]v, \[Eta]v]
};
expr = (expr /. {\[Chi]v -> \[Chi], \[Eta]v -> \[Eta]}) / pref // ExpandAll // Collect[#, f[\[Chi], \[Eta]], Simplify] &


restore\[Chi] = Solve[{
	\[Chi] x1Orth x2Orth == x12SqV + 2 x12OrthV
}, {x[1][4]}] // First
expr2 = expr /. restore\[Chi] /. x12Sq -> \[Chi] x1Orth x2Orth - 2 x12OrthV // Collect[#, f[\[Chi], \[Eta]], Simplify] &
...
randRules[args__] := (# -> RandomReal[WorkingPrecision->200]) & /@ {args}
fs = {f[\[Chi], \[Eta]], \!\(\*SuperscriptBox[\(f\), 
TagBox[
RowBox[{"(", 
RowBox[{"0", ",", "1"}], ")"}],
Derivative],
MultilineFunction->None]\)[\[Chi],\[Eta]], \!\(\*SuperscriptBox[\(f\), 
TagBox[
RowBox[{"(", 
RowBox[{"1", ",", "0"}], ")"}],
Derivative],
...
x[1][2] x[2][1] - x[1][1] x[2][2] > 0 /. rr
lhs = (Coefficient[expr, #] & /@ fs) /. {x12Sq -> x12SqV, x12Orth -> x12OrthV, x1Orth -> x1OrthV, x2Orth -> x2OrthV} /. rr;
rhs = (Coefficient[expr3, #] & /@ fs) /. {\[Chi] -> \[Chi]v, \[Eta] -> \[Eta]v} /. {x12Sq -> x12SqV, x12Orth -> x12OrthV, x1Orth -> x1OrthV, x2Orth -> x2OrthV} /. rr;
lhs - rhs // Expand
